...
var page = (function(window, document, $, M, undefined) {
    ...
    var PalletScan = function(plt_no){
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if (rowData.PLT_LOC_CD == "B01") {
                    popupManager.instance($("[data-lng='MSG.0000000684']").text(), {showtime:"LONG"}); // 이미 사외 공팔레트 입니다
                    $("#inputScan").focus();
                    return;
                }
                if (rowData.MAP_QTY > 0) {
                ...
                // Pallet 중복 스캔 방지
                $("#list_kd_250 .tableCont").each(function() {
                ...
                $("#list_kd_250_head").removeClass("blind");
                $("#list_kd_250").prepend(tag);
                $("#inputScan").focus();
            }
        });
    }
    ...
    var save = function() {
        ...
        $("#list_kd_250 .tableCont").each(function() {
            C1_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "PLT_NO":$(this).find(".PLT_NO").text(), "LOGI_CD":$("#selLOGI").val(), "USER_ID":getUSER_ID, "RTN_MSG":""});
        });
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_KD_250_C1.do',
            data: {
                'param1': C1_LIST
            },
            success: function(receivedData, setting) {
    ...
    var clear = function() {
        $("#list_kd_250").html("");
        $("#list_kd_250_head").addClass("blind");
