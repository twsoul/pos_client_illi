...
var page = (function(window, document, $, M, undefined) {
    ...
    var TMScan = function(TMBarCode){
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(rowData.OUT_FLAG == "N"){
                    popupManager.instance($("[data-lng='MSG.0000000489']").text(), {showtime:"LONG"}); // 출고된 TM이 아닙니다
                    $("#inputScan").focus();
                    return;
                }
                ...
                $("#list_pd_074_head").removeClass("blind");
                ...
                $("#list_pd_074").prepend(tag);

                C1_LIST.push({"VENDOR_CD":getVEND_CD, "TM_NO":TM_NO, "USER_ID":getUSER_ID, "RTN_MSG":""});
    ...
    // PR_PDA_PD_074_C1 호출 및 저장
    var save = function() {
        saveflag = true;
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_PD_074_C1.do',
            data: {
                'param1': C1_LIST
            },
            success: function(receivedData, setting) {
    ...
    var clear = function() {
        ...
        $("#list_pd_074").html("");
        $("#list_pd_074_head").addClass("blind");
