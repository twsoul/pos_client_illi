...
var page = (function(window, document, $, M, undefined) {
    ...
    // 스캔 시 부품식별표 조회
    var MetroScan = function(metro_no) {
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/OT_150_S1.do',
            data: {
            ...
            success: function(receivedData, setting) {
                ...
                if(receivedData.ListCount == 0){
                    popupManager.instance($("[data-lng='MSG.0000000716']").text(), {showtime:"LONG"}); // 존재하지 않는 METRO NO 입니다
                    $("#inputScan").focus();
                    return;
                }
                ...
                if(rowData.USE_FLAG != "Y"){
                    popupManager.instance($("[data-lng='MSG.0000000717']").text(), {showtime:"LONG"}); // 사용할 수 없는 메트로입니다
                    $("#inputScan").focus();
                    return;
                }
                // 스캔 시 플랜트, 저장위치 변경 불가
                ...
                $("#txtMETRO").val(metro_no);
                $("#txtYARD").val(rowData.YARD_NO);
                $("#txtQTY").val(rowData.FULL_QTY);
                $("#txtSPEC").val(rowData.SPEC_DESC);
                $("#txtREMARK").val(rowData.RMK);
    ...
    var save = function() {
        ...
        C1_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "METRO_NO":$("#txtMETRO").val(), "USER_ID":getUSER_ID, "RTN_MSG":""});

        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_OT_150_C1.do',
            data: {
                'param1': C1_LIST
            },
            success: function(receivedData, setting) {
