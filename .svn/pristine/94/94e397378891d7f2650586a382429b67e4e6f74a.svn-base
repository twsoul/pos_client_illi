...
var page = (function(window, document, $, M, undefined) {
    ...
    // 비축위치 콤보박스 정보 조회
    var SaveLocReq = function() {
        networkManager.httpSend({
            server: saveUserCo,
        	path: 'api/PD_096_S1.do',
        	data: {
            	'LANG' : getLNG,
        	...
        	success: function(receivedData, setting) {
        	    ...
        	    if(receivedData.ListCount != 0){
                     $.each(receivedData.ListData, function(index,rowData){
                        tag += "<option value='" + rowData.SAVE_LOC_CD + "'>" + rowData.SAVE_LOC_NM + "</option>";
                     });
                }
                $("#selSVLOC").append(tag);
        	}
    ...
    var TmScan = function(tm_no){
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                $("#list_pd_096_head").removeClass("blind");
                $("#list_pd_096").prepend(tag);
    ...
    var save = function() {
        ...
        $("#list_pd_096 .tableCont").each(function() {
            C2_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "SAVE_LOC":$("#selSVLOC").val(), "PLT_NO":$("#txtPALLET").text(), "TM_NO":$(this).find(".TM_NO").text(), "USER_ID":getUSER_ID, "RTN_MSG":""});
        });

        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_PD_096_C1.do',
            data: {
                'param1': C1_LIST,
                'param2': C2_LIST
            },
    ...
    var clear = function() {
        $("#list_pd_096").html("");
        $("#list_pd_096_head").addClass("blind");
        $("#txtPALLET").text("");
        $("#txtPART_CD").text("");
        $("#txtPACK_QTY").text("");
