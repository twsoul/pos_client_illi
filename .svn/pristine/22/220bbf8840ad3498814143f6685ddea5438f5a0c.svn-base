...
var page = (function(window, document, $, M, undefined) {
    ...
    var PalletScan = function(plt_no){
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if (rowData.PLT_LOC_CD != "B01" && rowData.PLT_LOC_CD != "C02") {
                    popupManager.instance($("[data-lng='MSG.0000000661']").text(), {showtime:"LONG"}); // 사용할 수 없는 팔레트입니다
                    $("#inputScan").focus();
                    return;
                }
                if(receivedData.ListCount == rowData.PACK_QTY){
                ...
                if(rowData.TM_NO != null){
                    ...
                    $("#list_kd_265_head").removeClass("blind");
                    $("#list_kd_265").prepend(tag);
                }
                $("#inputScan").focus();
            }
        });
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(rowData.USE_FLAG == "N"){
                    popupManager.instance($("[data-lng='MSG.0000000676']").text(), {showtime:"LONG"}); // 출하 처리된 TM입니다
                    $("#inputScan").focus();
                    return;
                }
                ...
                // TM 중복 스캔 방지
                $("#list_kd_265 .tableCont").each(function() {
                ...
                if(!return_flag && $("#inpBoxQty").text() == "0" && rowData.RETURN_FLAG == "Y"){
                    return_flag = true;
                }

                if(return_flag){
                    if(rowData.RETURN_FLAG == "N"){
                        popupManager.instance($("[data-lng='MSG.0000000762']").text(), {showtime:"LONG"}); // 고객사 반품용 팔레트 입니다. 고객사에서 반품된 TM을 스캔하십시오
                        $("#inputScan").focus();
                        return;
                    }
                } else if(!return_flag){
                    if(rowData.RETURN_FLAG == "Y"){
                        popupManager.instance($("[data-lng='MSG.0000000763']").text(), {showtime:"LONG"}); // 고객사 반품용 TM 입니다
                        $("#inputScan").focus();
                        return;
                    }
                }
    ...
    var save = function() {
        ...
        C1_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "PLT_NO":$("#txtPALLET").text(), "USER_ID":getUSER_ID, "RTN_MSG":""});
        $("#list_kd_265 .tableCont").each(function() {
            C2_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "PLT_NO":$("#txtPALLET").text(), "TM_NO":$(this).find(".TM_NO").text(), "LOGI_CD":$("#selLOGI").val(), "RET_YN":RETURN, "USER_ID":getUSER_ID, "RTN_MSG":""});
        });

        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_KD_265_C1.do',
            data: {
                'param1': C1_LIST,
                'param2': C2_LIST
            },
    ...
    var clear = function() {
        $("#list_kd_265").html("");
        $("#list_kd_265_head").addClass("blind");
        $("#txtPALLET").text("");
        $("#txtPACK_QTY").text("");
