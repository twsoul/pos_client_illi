...
var page = (function(window, document, $, M, undefined) {
        ...
        if(inputScan.length > 0) {
            if($("#txtORDR_NO").text()== ""){
                ...
                if($("#inpBoxNo").text()== ""){
                    ...
                    $("#inpBoxNo").text(box_no);
                 }else{
                    serial_no = serialNoProcessing(inputScan);
                    SerialNoInfo(serial_no);
                 }
            }
        }
    }
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                $.each(receivedData.ListData, function(index,rowData){
                    ...
                    $("#list_pd_046_head").removeClass("blind");
    ...
    // 시리얼 조회 함수
    var SerialNoInfo = function(SerialNo){
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PD_046_S2.do',
            data: {
                'PLANT_CD':$("#selPLANT").val(),
                'BOX_NO':$("#inpBoxNo").text(),
                'SERIAL_NO':SerialNo,
                'event':'팔레트 조회'
            },
            success: function(receivedData, setting) {
                ...
                if(receivedData.ListCount == 0){
                    popupManager.instance($("#inpBoxNo").text()+" "+$("[data-lng='MSG.0000000551']").text(), {showtime:"LONG"}); // 박스에 존재하지 않는 시리얼입니다
                    $("#inputScan").focus();
                    return;
                }
                if(rowData.PLANT_CD != $("#selPLANT").val()){
                ...
                if(box_no_exists){
                    ...
                    $("#list_pd_046 .tableCont").each(function() {
                ...
                if(!part_cd_exists){
                    ...
                    Bar_No_List.push({"COPORATE_CD":rowData.COPORATE_CD,"PLANT_CD":$("#selPLANT").val(),"LOC_TP":$("#selLOCTP").val(), "MOVE_SEQ": move_seq, "MOVE_NO":$("#txtORDR_NO").text(), "BOX_NO":rowData.BOX_NO, "SERIAL_NO":rowData.SERIAL_NO, "USER_ID":getUSER_ID, "RTN_MSG":""});
                }
                $("#inputScan").focus();
            }
        });
    ...
    // 바코드 Box_No 추출 함수
    var boxBarProcessing = function(box_bar_no){ // 바코드 예시 - [)>*06 VPF30 P88332MU121 S E T100011012021110101@4EA M C
    ...
    // PR_PDA_PD_046_C1 호출 및 저장
    var save = function() {
        saveflag = true;
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_PD_046_C1.do',
            data: {
                'param1': Bar_No_List
            },
            success: function(receivedData, setting) {
    ...
    var clear = function() {
        ...
        $("#txtORDR_NO").text("");
        $("#inpBoxNo").text("");
        $("#list_pd_046_head").addClass("blind");
