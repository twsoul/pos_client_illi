...
var page = (function(window, document, $, M, undefined) {
    ...
    var CaseNoScan = function(casebarno) {
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(rowData.HOLD_FLAG == "N"){
                    popupManager.instance($("[data-lng='MSG.0000000747']").text(), {showtime:"LONG"}); // 보류 상태의 CASE가 아닙니다
                    $("#inputScan").focus();
                    return;
                }
                ...
                $("#list_st_130_head").removeClass("blind");
    ...
    var BoxNoScan = function(box_bar_no) {
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(rowData.HOLD_FLAG == "N"){
                    popupManager.instance($("[data-lng='MSG.0000000438']").text(), {showtime:"LONG"}); // 보류상태의 부품식별표가 아닙니다
                    $("#inputScan").focus();
                    return;
                }
    ...
    var save = function() {
        ...
        $("#list_st_130 .tableCont").each(function() {
            C1_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "NUMBER":$(this).find(".NUMBER").text(), "PART_CD":$(this).find(".PART_CD").text(), "UNIT":$(this).find(".UNIT").text(), "USER_ID":getUSER_ID, "USER_NM":getUSER_NM,"RTN_MSG":""});
        });

        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_ST_130_C1.do',
            data: {
                'param1': C1_LIST
            },
            success: function(receivedData, setting) {
    ...
    var clear = function() {
        $("#list_st_130").html("");
        $("#list_st_130_head").addClass("blind");
