...
var page = (function(window, document, $, M, undefined) {
    ...
    var PalletScan = function(plt_no){
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if (rowData.INSERT_FLAG == "N") {
                    popupManager.instance($("[data-lng='MSG.0000000761']").text(), {showtime:"LONG"}); // 포장장이동 처리되지 않은 Pallet입니다
                    $("#inputScan").focus();
                    return;
                }
                if (rowData.PACK_FLAG == "Y") {
                    popupManager.instance($("[data-lng='MSG.0000000785']").text(), {showtime:"LONG"}); // 이미 포장 완료된 BOX입니다
                    $("#inputScan").focus();
                    return;
                }
                var exists = false;
               ...
               if(!exist){
                   tag += template.replace(/\{\{PART_CD\}\}/, rowData.PART_CD)
                                  .replace(/\{\{SCAN_QTY\}\}/, rowData.MAP_QTY)
                   $("#inpBoxQty").text(parseInt($("#inpBoxQty").text())+1);
                   $("#list_kd_220").prepend(tag);
               }
    ...
    var clear = function() {
        ...
        $("#list_kd_220").html("");
        $("#list_kd_220_head").addClass("blind");
