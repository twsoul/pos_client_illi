...
var page = (function(window, document, $, M, undefined) {
    ...
    var CaseNoScan = function(caseno) {
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(receivedData.ListCount == 0){
                    notExistCaseNoScan(caseno);
                    return;
                }

                // CASE 중복 스캔 방지
                $("#list_kd_280 .tableCont").each(function() {
                ...
                $("#list_kd_280_head").removeClass("blind");
    ...
    // ASN 정보 확인
    var notExistCaseNoScan = function(caseno) {
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/KD_280_S2.do',
            data: {
                'COPORATE_CD':getCORP_CD,
                'PLANT_CD':$("#selPLANT").val(),
                'LOC_TP':$("#selLOCTP").val(),
            ...
            success: function(receivedData, setting) {
                ...
                // U_LO_CASE_MST_MA, U_LO_IMP_BL_DET_MA 둘다 데이터 없을 때
                if(receivedData.ListCount == 0){
                ...
                if(Number(rowData.TOAL_QTY) < Number(rowData.BAR_QTY)){
                    popupManager.instance($("[data-lng='MSG.0000001021']").text(), {showtime:"LONG"}); // 재고가 없는 케이스입니다
                    $("#inputScan").focus();
                    return;
                }
                ...
                $.each(receivedData.ListData, function(index,rowData){
                    tag += template.replace(/\{\{GUBUN\}\}/, "CASE")
                                   .replace(/\{\{NO\}\}/, caseno)
                                   .replace(/\{\{PART_CD\}\}/, rowData.PART_CD)
                                   .replace(/\{\{BAR_QTY\}\}/, Number(rowData.BAR_QTY))
                                   .replace(/\{\{VENDOR_CD\}\}/, rowData.VENDOR_CD);
                });
                $("#list_kd_280").prepend(tag);
                $("#inpBoxQty").text(parseInt($("#inpBoxQty").text())+1);
    ...
    var clear = function() {
        saveflag = false;
        $("#list_kd_280").html("");
        $("#list_kd_280_head").addClass("blind");
        $("#inpBoxQty").text("0");
        $("#inpBoxQty2").text("0");
