...
var page = (function(window, document, $, M, undefined) {
	...
	var setInitEvent = function() {
        ...
        $("#chk_All").on("change", function() {
            if ($("#chk_All").prop("checked")) { // 체크됨
                $("#list_pd_085 .tableCont").each(function() {
                    $(this).addClass("on");
                });
            } else { // 체크 해제됨
                $("#list_pd_085 .tableCont").each(function() {
    ...
    var setSaveClickEvent = function(){
        ...
        if($("#selGUBUN").val() == null){
            popupManager.instance($("[data-lng='MSG.0000000530']").text(), {showtime:"LONG"}); // 비축구분을 선택해 주십시오
            $("#inputScan").focus();
            return;
        }

        if($("#list_pd_085 .tableCont").length == 0){
    ...
    var save = function(barcode) {
        ...
        networkManager.httpSend({
            ...
            data: {
                ...
                'GUBUN': '02',
                'BARCODE': barcode,
                'PROCESS_DATE': window.Utils.getTodayFormat("yyyyMMdd HH:mm:ss"),
                'AREA': area,
                'UID': getUSER_ID,
                'event':'비축 데이터 수신'
            },
