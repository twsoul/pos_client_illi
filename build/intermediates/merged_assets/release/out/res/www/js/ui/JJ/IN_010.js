...
var page = (function(window, document, $, M, undefined) {
    ...
    // 납입문서 조회 함수
    var DeliBarScan = function(deli_bar_no,plant_cd){
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/IN_010_S1.do',
            data: {
                'PACK_ORDR_NO':deli_bar_no,
                'PLANT_CD':plant_cd,
                'event':'납입문서 조회'
            },
            success: function(receivedData, setting) {
                ...
                $.each(receivedData.ListData, function(index,rowData){
                    tag += template.replace(/\{\{PLANT_CD\}\}/gi, rowData.PLANT_CD)
                                   ...
                                   .replace(/\{\{DELI_BAR_NO\}\}/gi, rowData.DELI_BAR_NO)
                                   .replace(/\{\{PART_CNT\}\}/, rowData.PART_CNT)
                                   .replace(/\{\{MOVE\}\}/, rowData.MOVE)
                                   .replace(/\{\{STATUS\}\}/, rowData.STATUS)
                                   .replace(/\{\{PART_CD\}\}/, rowData.PART_CD)
                                   .replace(/\{\{VEND_CD\}\}/, rowData.VEND_CD)
                ...
                $("#list_in_010").html("");
                $("#list_in_010_head").removeClass("blind");
                $("#list_in_010").append(tag);
                setListRowEvent();
