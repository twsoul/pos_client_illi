...
var page = (function(window, document, $, M, undefined) {
    ...
    var PalletScan = function(plt_no){
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if (rowData.PLT_LOC_CD != "B01" && rowData.PLT_LOC_CD != "B02" && rowData.PLT_LOC_CD != "A03") {
                ...
                if(rowData.TM_NO != null){
                    ...
                    $("#list_kd_260_head").removeClass("blind");
                    $("#list_kd_260").prepend(tag);
                }
                $("#inputScan").focus();
            }
        });
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(rowData.HOLD_FLAG == "Y") {
                    popupManager.instance($("[data-lng='MSG.0000000454']").text(), {showtime:"LONG"}); // 보류상태의 TM입니다
                    $("#inputScan").focus();
                    return;
                }
                ...
                // TM 중복 스캔 방지
                $("#list_kd_260 .tableCont").each(function() {
    ...
    var save = function() {
        ...
        $("#list_kd_260 .tableCont").each(function() {
            C2_LIST.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "PLT_NO":$("#txtPALLET").text(), "TM_NO":$(this).find(".TM_NO").text(), "LOGI_CD":$("#selLOGI").val(), "USER_ID":getUSER_ID, "RTN_MSG":""});
        });

        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_KD_260_C1.do',
            data: {
                'param1': C1_LIST,
                'param2': C2_LIST
            },
    ...
    var clear = function() {
        $("#list_kd_260").html("");
        $("#list_kd_260_head").addClass("blind");
        $("#txtPALLET").text("");
        $("#txtPART_CD").text("");
        $("#txtPACK_QTY").text("");
