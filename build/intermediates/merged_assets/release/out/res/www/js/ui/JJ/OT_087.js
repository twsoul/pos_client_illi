...
var page = (function(window, document, $, M, undefined) {
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                $.each(receivedData.ListData, function(index,rowData){
                    ...
                    $("#list_ot_087_head").removeClass("blind");
                    tag += template.replace(/\{\{MOVE_SEQ\}\}/, rowData.MOVE_SEQ)
                                   .replace(/\{\{PART_CD\}\}/, rowData.PART_CD)
                                   .replace(/\{\{MOVE_QTY\}\}/, rowData.MOVE_QTY);
                    $("#list_ot_087").append(tag);
                });
                Box_No_List.push({"COPORATE_CD":getCORP_CD,"PLANT_CD": $("#selPLANT").val(),"LOC_TP":$("#selLOCTP").val(), "MOVE_NO":rowData.MOVE_NO, "USER_ID":getUSER_ID, "RTN_MSG":""});
                $("#inputScan").focus();
            }
        });
    };
    ...
    var clear = function() {
        ...
        $("#list_ot_087").html("");
        $("#txtORDR_NO").text("");
        $("#list_ot_087_head").addClass("blind");
