...
var page = (function(window, document, $, M, undefined) {
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                if(rowData.PRE_INSP_DTTM != null && rowData.PRE_INSP_DTTM != undefined && rowData.PRE_INSP_DTTM != "undefined"){
                    if(parseInt(rowData.PRE_INSP_DTTM) < pre_insp_dttm || pre_insp_dttm == 0){
                        pre_insp_dttm = parseInt(rowData.PRE_INSP_DTTM);
                    }
                }
                if(pre_insp_dttm != 0){ // PRE_INSP_DTTM값이 null인경우 ""값을 넘겨줌
                    strdttm = pre_insp_dttm.toString();
                }

                // 신규 식별표 생성 시
                if ($("#chk_Info").prop("checked")) {
                    if($("#txtBOX_NO").text() == ""){
                        $("#list_st_030_head").removeClass("blind");
                        $("#txtPART_CD").text(rowData.PART_CD);
                        $("#txtBARQTY").text(rowData.BAR_QTY);
                        $("#txtEXPQTY").text(rowData.BAR_QTY);
                        New_Box_No_List.push({"COPORATE_CD":rowData.COPORATE_CD, "PLANT_CD":$("#selPLANT").val(), "VENDOR_CD":rowData.VENDOR_CD, "RTN_MSG":"", "RTN_BOX":""})
                        NewBoxNoInfo();
                        Box_No_List_C3.push({"CHECK_YN":"Y", "COPORATE_CD":rowData.COPORATE_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "N_BOX_NO":$("#txtBOX_NO").text(), "BOX_NO":rowData.BOX_NO, "VENDOR_CD":rowData.VENDOR_CD, "PART_CD":rowData.PART_CD, "EXP_QTY":"", "PRE_INSP_DTTM":"", "INSP_FLAG":rowData.INSP_FLAG, "USER_ID":getUSER_ID, "USER_NM":getUSER_NM, "RTN_MSG":""});
                    } else{
                        var barqty = 0;
                    ...
                    $("#list_st_030").prepend(tag);
                    Box_No_List_C2.push({"COPORATE_CD":rowData.COPORATE_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "N_BOX_NO":$("#txtBOX_NO").text(), "BOX_NO":rowData.BOX_NO, "VENDOR_CD":rowData.VENDOR_CD, "PART_CD":rowData.PART_CD, "MRG_QTY":rowData.BAR_QTY, "USER_ID":getUSER_ID, "USER_NM":getUSER_NM, "RTN_MSG":""});
                } else { // 기존 식별표와 병합시
                    if($("#txtBOX_NO").text() == ""){
                        $("#list_st_030_head").removeClass("blind");
                        ...
                        $("#txtBARQTY").text(rowData.BAR_QTY);
                        $("#txtEXPQTY").text(rowData.BAR_QTY);
                        Box_No_List_C3.push({"CHECK_YN":"N", "COPORATE_CD":rowData.COPORATE_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "N_BOX_NO":$("#txtBOX_NO").text(), "BOX_NO":rowData.BOX_NO, "VENDOR_CD":rowData.VENDOR_CD, "PART_CD":rowData.PART_CD, "EXP_QTY":"", "PRE_INSP_DTTM":"", "INSP_FLAG":rowData.INSP_FLAG, "USER_ID":getUSER_ID, "USER_NM":getUSER_NM, "RTN_MSG":""});
    ...
    var setSaveClickEvent = function(){
        ...
        $.each(Box_No_List_C3,function(key){
            Box_No_List_C3[key]['PRE_INSP_DTTM'] = strdttm;
        });
    ...
    var clear = function() {
        ...
        $("#list_st_030_head").addClass("blind");
    ...
    var IsNotExitList = function(box_no){
        ...
        if ($("#chk_Info").prop("checked")) {
            $("#list_st_030 .tableCont").each(function() {
