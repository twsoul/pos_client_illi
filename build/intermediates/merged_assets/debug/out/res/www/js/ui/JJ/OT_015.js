...
var page = (function(window, document, $, M, undefined) {
    ...
    var setInitEvent = function() {
        ...
        $("#selLOCTP").on("change", function() {
            var query = 'SELECT BOXBAR FROM OUTBOX WHERE OUTTYPE = "20" AND PLANTCD = "'+$("#selPLANT").val()+'" AND LOCTP = "'+$("#selLOCTP").val()+'" ORDER BY CREATE_DT';
        ...
        $("#chk_Info").on("change", function() {
            if ($("#chk_Info").prop("checked")) { // 체크됨
                $("#list_ot_015_1").parent().addClass("blind");
                $("#list_ot_015_2").parent().removeClass("blind");
                clear();
            } else { // 체크 해제됨
                $("#list_ot_015_1").parent().removeClass("blind");
                $("#list_ot_015_2").parent().addClass("blind");
                clear();
                var query = 'SELECT BOXBAR FROM OUTBOX WHERE OUTTYPE = "20" AND PLANTCD = "'+$("#selPLANT").val()+'" AND LOCTP = "'+$("#selLOCTP").val()+'" ORDER BY CREATE_DT';
                M.db.execute(getUSER_ID, query, function(status, result,  name) {
                    if(status == "FAIL") {
    ...
    var deleteTABLE = function() {
        var query = 'DELETE FROM OUTBOX WHERE OUTTYPE = "20" AND PLANTCD = "'+$("#selPLANT").val()+'" AND LOCTP = "'+$("#selLOCTP").val()+'"';
        M.db.execute(getUSER_ID, query, function(status, result, name) {
            if(status == "FAIL") {
                popupManager.alert($("[data-lng='MSG.0000000196']").text(), {
                	title: $("[data-lng='MSG.0000000004']").text(),
    		...
    		success: function(receivedData, setting) {
    		    ...
    		    if(receivedData.ListCount == 0){
                    ...
                    $.each(receivedData.ListData, function(index,rowData){
                    	tag += "<option value='" + rowData.VALUE + "' value1='" + rowData.SCM_FIFO_FLAG + "' value2='" + rowData.ETC_FLAG +"'>" + rowData.TEXT + "</option>";
                    });
                    $("#selPLANT").append(tag);
        ...
        networkManager.httpSend({
        	...
        	success: function(receivedData, setting) {
        	    ...
        	    if(type == "20") {
                    ...
                    if($("#selPLANT option:selected").val() == getWERKS){
                        $("#selLOCTP2").val(getLGORT20).prop("selected", true);
                    }
                }
        	}
        });
    ...
    var BoxNoScanTemp = function(DBlist) {
        networkManager.httpSend({
            server: saveUserCo,
            ...
            success: function(receivedData, setting) {
                ...
                $.each(receivedData.ListData, function(index,rowData){
                    ...
                    if(rowData.LOC_TP_TYPE != 10){
                        var query = 'DELETE FROM OUTBOX WHERE OUTTYPE = "20" AND BOXBAR = "'+rowData.BOX_BAR_NO+'"';
                        M.db.execute(getUSER_ID, query, function(status, result,  name) {
                        });
                        ...
                        tag += template.replace(/\{\{PART_NO\}\}/, rowData.PART_CD)
                                       ...
                                       .replace(/\{\{LOT_FIFO_FLAG\}\}/, rowData.LOT_FIFO_FLAG)
                        ...
                        var cnt = 0;
                        var cnt2 = 0;
                        $("#list_ot_015_1 .tableCont").each(function() {
                            if($(this).find(".PART_NO").text() == rowData.PART_CD){
                                if($(this).data("fifoflag")<rowData.FIFO_FLAG){
                                    cnt ++;
                                }
                                if($(this).data("lotfifoflag")<rowData.LOT_FIFO_FLAG){
                                    cnt2 ++;
                                }
                            }
                        });
                        $("#list_ot_015_1").prepend(tag);
                        if($("#selPLANT option:selected").attr('value1') == "Y"){
                            if(cnt2  < rowData.LOT_FIFO_FLAG){
                                $("#list_ot_015_1 .tableCont:eq(0)").data("lotfifo","Y");
                            }
                        }else{
                            if(rowData.FIFO_CHK == "Y" && cnt < rowData.FIFO_FLAG){
                                $("#list_ot_015_1 .tableCont:eq(0)").data("fifo","Y");
                            }
                        }
                    }
                });
                if(DBlist.length > 0){
                    ...
                    for(var i = 0; i < DBlist.length; i++ ) {
                        query += 'DELETE FROM OUTBOX WHERE OUTTYPE = "20" AND BOXBAR = "'+DBlist[i]+'";';
                    }
                    M.db.execute({
                        path:getUSER_ID,
                        sql:query,
                ...
                if($("#list_ot_015_1 .tableCont").length == 0) clear();
            }
        });
    };
    ...
    var BoxNoScan = function(box_barcode) {
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                console.log("SEL_FLAG : "+rowData.SEL_FLAG);
                if(rowData.SEL_FLAG == "Y"){
                    ...
                    buttons: [$("[data-lng='MSG.0000000002']").text(), $("[data-lng='MSG.0000000003']").text()] // 확인, 취소
                        ...
                        if (index == 1){
                            ...
                            console.log("SCM_FLAG : "+$("#selPLANT option:selected").attr('value1'));
                            if($("#selPLANT option:selected").attr('value1') == "Y"){
                                if(cnt2  < rowData.LOT_FIFO_FLAG){
                                    popupManager.alert("LOT"+$("[data-lng='MSG.0000000333']").text(), { // 선입선출에 위배 되었습니다 그래도 저장하시겠습니까?
                                    title: $("[data-lng='MSG.0000000004']").text(), // 알림
                                    buttons: [$("[data-lng='MSG.0000000002']").text(), $("[data-lng='MSG.0000000003']").text()] // 확인, 취소
                                    }, function(index) {
                                        if (index == 1){
                                            ...
                                            // 스캔마다 BOX 개수 증가
                                            $("#inpBoxQty").text(parseInt($("#inpBoxQty").text())+1);
                                            var query = 'INSERT OR REPLACE INTO OUTBOX (OUTTYPE, BOXBAR, PLANTCD, LOCTP) values ("20","'+rowData.BOX_BAR_NO+'","'+$("#selPLANT").val()+'","'+$("#selLOCTP").val()+'")';
                                            M.db.execute(getUSER_ID, query, function(status, result, name) {
                                                if(status == "FAIL") {
                                                    popupManager.alert($("[data-lng='MSG.0000000196']").text(), {
                                                    	title: $("[data-lng='MSG.0000000004']").text(),
                                            ...
                                            $("#list_ot_015_1").prepend(tag);
                                            console.log("tableCont 1 fifo : "+$("#list_ot_015_1 .tableCont:eq(0)").data("lotfifo"));
                                            $("#list_ot_015_1 .tableCont:eq(0)").data("lotfifo","Y");
                                            console.log("tableCont 1 fifo : "+$("#list_ot_015_1 .tableCont:eq(0)").data("lotfifo"));
                                            $("#inputScan").focus();
                                        }
                                    ...
                                    if(cnt < rowData.FIFO_FLAG){
                                        ...
                                        buttons: [$("[data-lng='MSG.0000000002']").text(), $("[data-lng='MSG.0000000003']").text()] // 확인, 취소
                                            ...
                                            if (index == 1){
                                                ...
                                                $("#list_ot_015_1").prepend(tag);
                                                console.log("tableCont 1 fifo : "+$("#list_ot_015_1 .tableCont:eq(0)").data("fifo"));
                                                $("#list_ot_015_1 .tableCont:eq(0)").data("fifo","Y");
                                                console.log("tableCont 1 fifo : "+$("#list_ot_015_1 .tableCont:eq(0)").data("fifo"));
                                                $("#inputScan").focus();
                                            }
                                ...
                                if(rowData.LOT_FIFO_FLAG != "0"){
                                    ...
                                    buttons: [$("[data-lng='MSG.0000000002']").text(), $("[data-lng='MSG.0000000003']").text()] // 확인, 취소
                                        ...
                                        if (index == 1){
                                            ...
                                            $("#list_ot_015_2").prepend(tag);
                                            console.log("tableCont 1 fifo : "+$("#list_ot_015_2 .tableCont:eq(0)").data("lotfifo"));
                                            $("#list_ot_015_2 .tableCont:eq(0)").data("lotfifo","Y");
                                            console.log("tableCont 1 fifo : "+$("#list_ot_015_2 .tableCont:eq(0)").data("lotfifo"));
                                ...
                                if(rowData.FIFO_CHK == "Y" && rowData.FIFO_FLAG != "0"){
                                    ...
                                    buttons: [$("[data-lng='MSG.0000000002']").text(), $("[data-lng='MSG.0000000003']").text()] // 확인, 취소
                                        ...
                                        if (index == 1){
                                            ...
                                            $("#list_ot_015_2").prepend(tag);
                                            console.log("tableCont 1 fifo : "+$("#list_ot_015_2 .tableCont:eq(0)").data("fifo"));
                                            $("#list_ot_015_2 .tableCont:eq(0)").data("fifo","Y");
                                            console.log("tableCont 1 fifo : "+$("#list_ot_015_2 .tableCont:eq(0)").data("fifo"));
                                            $("#txtBoxNo").text(rowData.BOX_NO);
                                            // 분할 수량 입력 포커스 이동
                                            $(".DIV_QTY").click();
    ...
    var setSaveClickEvent = function(){
        ...
        if ($("#chk_Info").prop("checked")) {
            $("#list_ot_015_2 .tableCont").each(function() {
                var pre_insp_dttm = "";
                if($(this).data("fifo") == "Y" || $(this).data("lotfifo") == "Y"){
                    fifo = "Y";
                }
                ...
                Box_No_List.push({"CHECK_YN":"Y", "COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "TO_LOC_TP":$("#selLOCTP2").val(), "BAN_CD":$("#selBAN").val(), "LINE_CD":$("#selLINE").val(), "BOX_NO":$(this).data("boxno"), "VENDOR_CD":$(this).data("vendcd"), "PART_CD":$(this).find(".PART_NO").text(), "EXP_QTY":$(this).find(".PRE_QTY").text(), "DIV_QTY":$(this).find(".DIV_QTY").val(), "PRE_INSP_DTTM":pre_insp_dttm, "FIFO":fifo, "INSP_FLAG":$(this).data("inspflag"), "WORK_NM":getUSER_NM, "WORK_ID":getUSER_ID, "RTN_MSG":""});
            });
        } else {
            $("#list_ot_015_1 .tableCont").each(function() {
                var pre_insp_dttm = "";
                console.log("fifo : "+$(this).data("fifo"));
                console.log("lotfifo : "+$(this).data("lotfifo"));
                if($(this).data("fifo") == "Y" || $(this).data("lotfifo") == "Y"){
                    fifo = "Y";
                }
                if($(this).data("preinspdttm") != null && $(this).data("preinspdttm") != undefined && $(this).data("preinspdttm") != "undefined"){
                ...
                Box_No_List.push({"CHECK_YN":"N", "COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "TO_LOC_TP":$("#selLOCTP2").val(), "BAN_CD":$("#selBAN").val(), "LINE_CD":$("#selLINE").val(), "BOX_NO":$(this).data("boxno"), "VENDOR_CD":$(this).data("vendcd"), "PART_CD":$(this).find(".PART_NO").text(), "EXP_QTY":$(this).find(".BAR_QTY").text(), "DIV_QTY":"0", "PRE_INSP_DTTM":pre_insp_dttm, "FIFO":fifo, "INSP_FLAG":$(this).data("inspflag"), "WORK_NM":getUSER_NM, "WORK_ID":getUSER_ID, "RTN_MSG":""});
            });
        }
    ...
    // PR_PDA_OT_015_C1 호출 및 저장
    var save = function(Box_No_List) {
        saveflag = true;
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_OT_015_C1.do',
            data: {
                'param1': Box_No_List
            },
            success: function(receivedData, setting) {
                if(receivedData.result=="S"){
                    ...
                    if(receivedData.resultLotNo != ""){ // LotNo가 xx(으)로 변경되었습니다
                        popupManager.alert($("[data-lng='MSG.0000000446']").text()+ " " + receivedData.resultLotNo + " " + $("[data-lng='MSG.0000000447']").text(), {
                        title: $("[data-lng='MSG.0000000004']").text(), // 알림
    ...
    var save_C2 = function(){
        var C2_LIST = [];
        C2_LIST.push({"PLANT_CD":$("#selPLANT").val(), "LINE_CD":$("#selLINE").val(), "WORK_ID":getUSER_ID, "RTN_MSG":""});
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_OT_015_C2.do',
            data: {
                'param1': C2_LIST
            },
            success: function(receivedData, setting) {
                if(receivedData.result=="S"){
                    ...
                    popupManager.instance($("[data-lng='MSG.0000000592']").text(), {showtime:"LONG"}); // 저장에 실패하였습니다 수동으로 현황판 알림을 확인해 주십시오
                    clear();
                }
            },
            error: function(errorCode, errorMessage, settings) {
