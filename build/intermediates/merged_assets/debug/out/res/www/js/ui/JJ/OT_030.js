...
var page = (function(window, document, $, M, undefined) {
    ...
    var setInitEvent = function() {
        ...
        $("#selLOCTP").on("change", function() {
            var query = 'SELECT BOXBAR, BAN, LINE FROM OUTBOX WHERE OUTTYPE = "31" AND PLANTCD = "'+$("#selPLANT").val()+'" AND LOCTP = "'+$("#selLOCTP").val()+'" ORDER BY CREATE_DT';
            ...
            success: function(receivedData, setting) {
                ...
                $.each(receivedData.ListData, function(index,rowData){
                    ...
                    if(rowData.LOC_TP_TYPE != 10){
                        ...
                        var cnt = 0;
                        $("#list_ot_030 .tableCont").each(function() {
                            if($(this).find(".PART_NO").text() == rowData.PART_CD && $(this).data("fifoflag") < rowData.FIFO_FLAG){
                                cnt ++;
                            }
                        });
                        $("#list_ot_030").prepend(tag);
                        $.each(data.row_list, function(index2,rowData2){
                            if(rowData2.BOXBAR == rowData.BOX_BAR_NO){
                                $("#list_ot_030 .tableCont:eq(0)").find(".selBAN").val(rowData2.BAN).prop("selected", true);
                                $("#list_ot_030 .tableCont:eq(0)").find(".selLINE").val(rowData2.LINE).prop("selected", true);
                                return false;
                            }
                        });
                        if(rowData.FIFO_CHK == "Y" && cnt < rowData.FIFO_FLAG){
                            $("#list_ot_030 .tableCont:eq(0)").data("fifo","Y");
                        }
                    }
                });
                if(DBlist.length > 0){
                                ...
                                if(cnt < rowData.FIFO_FLAG){
                                    ...
                                    buttons: [$("[data-lng='MSG.0000000002']").text(), $("[data-lng='MSG.0000000003']").text()] // 확인, 취소
                                        ...
                                        if (index == 1){
                                            ...
                                            $("#list_ot_030").prepend(tag);
                                            if(banVal != "" && lineVal != ""){
                                                $("#list_ot_030 .tableCont:eq(0)").find(".selBAN").val(banVal).prop("selected", true);
                                                $("#list_ot_030 .tableCont:eq(0)").find(".selLINE").val(lineVal).prop("selected", true);
                                            }
                                            var query = 'INSERT OR REPLACE INTO OUTBOX (OUTTYPE, BOXBAR, PLANTCD, LOCTP, BAN, LINE) values ("31","'+rowData.BOX_BAR_NO+'","'+$("#selPLANT").val()+'","'+$("#selLOCTP").val()+'","'+$("#list_ot_030 .tableCont:eq(0)").find(".selBAN").val()+'","'+$("#list_ot_030 .tableCont:eq(0)").find(".selLINE").val()+'")';
                                            M.db.execute(getUSER_ID, query, function(status, result, name) {
                                                if(status == "FAIL") {
                                                    popupManager.alert($("[data-lng='MSG.0000000196']").text(), {
                                                    	title: $("[data-lng='MSG.0000000004']").text(),
    ...
    var sel_Line_Set = function(obj){
        ...
        networkManager.httpSend({
            ...
            success: function(receivedData, setting) {
                ...
                var query = 'UPDATE OUTBOX SET BAN = "'+$(obj).val()+'", LINE = "'+$(obj).parent().parent().siblings().find(".selLINE").val()+'" WHERE OUTTYPE = "31" AND BOXBAR = "'+box_bar_no+'"';
                M.db.execute(getUSER_ID, query, function(status, result, name) {
                });
            }
        });
    ...
    var setSaveClickEvent = function(){
        ...
        $("#list_ot_030 .tableCont").each(function() {
            ...
            if(plantChk == "Y"){ // 반, 라인 입력이 필수 인지 체크
                if($(this).find(".selBAN").val() != null && $(this).find(".selLINE").val() != null){
                    Box_No_List.push({"COPORATE_CD":getCORP_CD, "PLANT_CD":$("#selPLANT").val(), "LOC_TP":$("#selLOCTP").val(), "TO_LOC_TP":$("#selLOCTP2").val(), "BAN_CD":$(this).find(".selBAN").val(),"LINE_CD":$(this).find(".selLINE").val(), "BOX_NO":$(this).data("boxno"), "VENDOR_CD":$(this).data("vendcd"), "PART_CD":$(this).find(".PART_NO").text(), "SCAN_QTY":$(this).find(".SCAN_QTY").text(), "FIFO":fifo,"USER_ID":getUSER_ID, "USER_NM":getUSER_NM, "RTN_MSG":""});
                } else {
    ...
    // PR_PDA_OT_030_C1 호출 및 저장
    var save = function(Box_No_List) {
        saveflag = true;
        networkManager.httpSend({
            server: saveUserCo,
            path: 'api/PR_PDA_OT_030_C1.do',
            data: {
                'param1': Box_No_List
            },
            success: function(receivedData, setting) {
